...
def main():
    ...
    parser.add_argument('-x', help="The size in the X dimension (Angstroms)", type=int)
    parser.add_argument('-y', help="The size in the X dimension (Angstroms)", type=int)
    parser.add_argument('-z', help="The size in the X dimension (Angstroms)", type=int)
    ...
    try:
        ...
        utility.debug_and_exit(f'Receptor preparation complete in {t.split(".")[0]}\n', task=args.task, status=60,
                               grid=wd / f'{slug}.maps.fld', rigid=wd / f'{slug}.pdbqt')
