...
def main():
    ...
    cmd = ['batch-ligand', args.ligand, f'--outdir {args.outdir}', f'--batch {args.batch}', f'--task {args.task}']
    if args.debug:
        cmd.append('--debug')
    cmd = ' \\\n  '.join(cmd)
    code, job_id = vstool.submit(env + cmd,
                                 ...
                                 log='%x.%j.log', script='batch.ligand.sh', delay=args.delay)
    if not job_id:
        vstool.error_and_eixt('Failed to submit batch ligand job, cannot continue', ta=args.task, status=-10)
    ...
    code, job_id = vstool.submit(env + ' \\\n  '.join(cmd),
                                 ...
                                 partition=args.partition, email=args.email, mail_type=args.email_type,
                                 log='%x.%j.%A.%a.log', script='docking.sh',
